    ...
    <div class="tab-content">
          ...
          ?>
              <div class="card" style="border-radius: 5px;">
                ...
                <div style="padding: 10px;">
                  <img src="../<?php echo $rows['image']; ?>" alt="<?php echo $rows['image']; ?>" style="height:200px; width: 100%; border-radius: 5px;">
                </div>
      ...
      <div id="menu4" class="tab-pane fade">
        <div class="container">
          ...
          <div class="col-xs-6 col-xs-offset-3" style="background: #f1f1f1;">
            <?php
            $owner_email = $_SESSION["email"];
            $ownerQ = $db->query("SELECT * from owner where email='$owner_email'");
            if ($ownerQ) {
              $ownerRow = mysqli_fetch_assoc($ownerQ);
              $owner_rand_id = $ownerRow['owner_rand_id'];

              $messageQ = $db->query("SELECT DISTINCT send_id FROM message WHERE receive_id = '$owner_rand_id'");
              if (mysqli_num_rows($messageQ) > 0) {
                while ($messageR = mysqli_fetch_assoc($messageQ)) {
                  // $messageR = mysqli_fetch_assoc($messageQ);
                  $tenant_rand_id = $messageR['send_id'];

                  $tenantQ = $db->query("SELECT * from tenant where tenant_rand_id='$tenant_rand_id'");
                  if ($tenantQ) {
                    while ($tenantRow = mysqli_fetch_assoc($tenantQ)) { ?>
                      <form class="message_list" method="post" style="border-bottom: 1px solid #999;" action="chat/message.php">
                        <ul>
                          <li>
                            <?php if ($tenantRow['image'] != "") {  ?>

                              <img src="../<?= $tenantRow['image'] ?>" alt="<?= $tenantRow['image'] ?>" style="height: 80px; width:80px; border-radius:50%; margin-right: 20px; margin-top: 10px;">

                            <?php } else {  ?>

                              <img src="../images/avater.png" alt="avater.png" style="height: 80px; width:80px; border-radius:50%; margin-right: 20px; margin-top: 10px;">

                            <?php } ?>
                            <strong><?= $tenantRow["full_name"] ?></strong>
                            <input type="submit" value="View Message" class="btn btn-success" style="margin-left: 10px;">
                            <input type="text" value="<?= $tenantRow["tenant_rand_id"] ?>" name="tenant_rand_id" hidden>
                          </li>
                        </ul>
                      </form>
            ...
            <?php }
                ...
                echo "<h2>No message Available Here</h2>";
              }
            } ?>
          </div>
        </div>
            ...
            <table id="myTable" class="table table-bordered"  style="width: 100%;">
              ...
              ?>
                      <tr>
                        ...
                        <td>taka.<?php echo $rows['estimated_price'] ?></td>
                        ...
                        <td  style="width: 10%;"><?php echo $rows['description'] ?></td>
                        <td>
                          ...
                          if (mysqli_num_rows($query) > 0) {
                            while ($row = mysqli_fetch_assoc($query)) { ?>
                              <img src="<?php echo $row['p_photo'] ?>" width="50px" style="margin-top: 5px;">
                  <?php }
                          }
                        }
                      }
            ...
            <table id="myTable" class="table table-bordered">
              ...
              ?>
                  <tr>
                    ...
                    <form method="POST">
                      <td>
                        <?php if ($rows['booked'] == "No") { ?>

                          <a class="btn btn-success" href="property-update-delete.php?update_id=<?= $rows['property_id'] ?>" target="_blank"> Update </a>
                          <a class="btn btn-danger" href="property-update-delete.php?delete_id=<?= $rows['property_id'] ?>" style="margin-top: 5px;" onclick="return confirm('Do you really want to Delete this property')"> Delete </a>

                        <?php } else { ?>
                          <font color="red"> Booking process runing Or Booked</font>
                        <?php } ?>
          ...
          <table id="myTable" class="table table-bordered">
            ...
            ?>
                            <tr>
                              ...
                              <td>
                                <?php $sql2 = "SELECT * from property_photo where property_id='$property_id' LIMIT 1";
                                $query = mysqli_query($db, $sql2);

                                if (mysqli_num_rows($query) > 0) {
                                  while ($row = mysqli_fetch_assoc($query)) { ?>
                                    <img src="<?php echo $row['p_photo'] ?>" width="80px">
                                <?php }
                                }  ?>
                              </td>
                              <td>
                                <?php
                                if ($ro['booked'] == "Pending") { ?>

                                  <a href="request-order.php?payment_confirm_id=<?= $property_id; ?>" class="btn btn-success">Confirm</a>
                                  <a href="request-order.php?payment_cancel_id=<?= $property_id; ?>" class="btn btn-danger" style="margin-top: 5px;">Cancel</a>

                                <?php } elseif ($ro['booked'] == "Yes") { ?>

                                  <span style="color:red">Payment Successful</span>
